...
```bash
./gradlew build
# 결과물: build/libs/WorldBackUp-1.1.1.jar

# 서버 plugins 폴더로 바로 복사하려면
./gradlew deployPlugin -PserverDir="D:/minecraft/server"
```

`build/libs/WorldBackUp-1.1.1.jar` 를 서버의 `plugins/` 폴더에 넣고 재시작하면
...
> 10초를 넘으면 주기를 더 줄이는 것은 무리입니다. 주기를 늘리고 첫 계단의 `keep` 만 줄이면 됩니다.
>
