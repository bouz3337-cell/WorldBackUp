...
| **`03:00`** | 가장 가까운 과거의 그 시각 **이전** 백업 |
| `2026-08-16 03:00` | 절대 시각 **이전** 백업 |
...
| `retention.tiers` | `[]` | **계단식 보관.** 설정하면 아래 3개를 대신함 |
...
retention:
  tiers:
    ...
    - { every: 1h,  keep: 18 }   # 1시간마다 → 24시간   ← 자는 동안
    - { every: 6h,  keep: 12 }   # 6시간마다 → 3일
    - { every: 24h, keep: 14 }   # 하루 1개  → 14일
```

**지난 하루가 1시간 해상도**입니다. 새벽 3시 사고를 정오에 발견해도 2시대 백업으로 돌아가
잃는 것이 최대 1시간입니다. 15분 주기 기준 총 68개로 약 18일이 덮입니다.
