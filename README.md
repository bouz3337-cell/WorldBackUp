...
```bash
./gradlew build
# 결과물: build/libs/WorldBackUp-1.5.0.jar

# 서버 plugins 폴더로 바로 복사하려면
./gradlew deployPlugin -PserverDir="D:/minecraft/server"
```

`build/libs/WorldBackUp-1.5.0.jar` 를 서버의 `plugins/` 폴더에 넣고 재시작하면
...
- `OneBackTest` — **아카이브 하나로 서버를 다시 열 수 있는가.** 실제로 아카이브를 만들어
  ...
  되살아나지 않는지도 함께 봅니다
- `OneBackDisasterTest` — 위가 **목록**을 본다면, 이쪽은 그다음 질문입니다: 목록이 맞다고
  되살아난다는 뜻은 아닙니다. 그래서 **원본 서버 폴더를 지우고** zip 하나만으로 빈 자리에
  다시 세운 뒤, 지우기 전에 떠 둔 사본과 **차집합이 비어 있는지**로 확인합니다 — 파일 이름을
  적어 두고 세는 방식은 나중에 생긴 파일을 놓치기 때문입니다. 푸는 쪽은 이 플러그인이 아니라
  평범한 zip 읽기입니다(재난 때 관리자가 쓰는 것이 그것이므로). region 파일에 0~255를 그대로
  담아 **이진 파일이 한 바이트도 안 달라지는지**, zip 안의 경로가 상위 폴더를 가리키지 않는지,
  그리고 **서버 폴더 안에만 둔 아카이브는 서버와 함께 사라진다는 것**까지 못 박습니다
- `PluginDataRoundTripTest` — **다른 플러그인의 데이터가 되돌리면 그대로 돌아오는지.** 실서버 모양
  (MineProtect·mopi·EventSystem + 월드)을 만들어 놓고 백업 → 사고 → 복원을 통째로 돌립니다.
  경제 잔고가 날아가고 보호구역이 지워지고 플러그인 폴더가 통째로 사라진 상황에서 되돌아오는지,
  **차등 백업에서도** 그런지(담기지 않은 파일을 기준 백업에서 꺼내 오는 이음매), jar 는 덮어쓰지
  않는지, 백업 뒤 설치한 플러그인의 데이터가 `replaced/` 로 옮겨지는지까지 봅니다
...
- `UserListSyncTest` — 위 판단을 **살아 있는 서버에 반영하는 쪽**. 이 플러그인을 고치게 만든 사고가
  정확히 여기였습니다 — 파일은 되돌아갔는데 op 목록은 그대로였던 것. 가짜 서버가 **`setOp` 때마다
  파일을 덮어쓰는 것까지** 흉내 내므로, 맞추는 과정에서 되돌린 이름과 `level` 이 깎여 나가지 않는지도
  같이 확인됩니다. 목록 하나가 실패해도 나머지가 맞춰지는지도 봅니다 — 여기서 예외가 나가면 복원
  직후, 되돌릴 수단이 가장 필요한 순간에 백업 플러그인이 꺼진 서버가 됩니다
